...
This Makefile target will instruct the ```make``` utility to invoke ```flex``` using the following command;

```
flex --verbose --header-file=SimpleScanner.h --outfile=SimpleScanner.c SimpleScanner.l
```

Note that the output from this command will be a .c file which is called ```SimpleScanner.c```. This file will contain a C language implementation of the Scanner which has been generated by ```flex```.
...
Note that the output from this command will be a .cpp file which is called ```SimpleScanner.cpp```. This file will contain a C++ language implementation of the Scanner which has been generated by ```flex```.


## 5) Using the built package.

Change into the directory which the package has been installed into. From there, then change into
the ```bin``` sub directory. Within this directory, you should find an executable file which is
called either;

```
SimpleScannerC or SimpleScannerCpp
```
...
As you can see from this code listing, the Parser just keeps asking the Scanner for tokens until the Scanner either runs out of tokens, or returns an invalid token.
